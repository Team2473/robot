...
public class SemiAuto {
	...
	public static void autoLoop() {
		...
		// Calculate outputs
		// TODO: isDown() and isUp() should be used to restrict speed during
		// drive
		if (currentAuto == AutoState.START) {
			Shooter.getInstance().setPosition(90);
			encStart = Motor.getInstance().getEncFL();
			if (Motor.getInstance().getEncFL() == -75) { // change to encoder
															// value
				currentAuto = AutoState.DOWN;
			} else {
				// do nothing
	...
	public static boolean wallDetected() {
		return Telemetry.getInstance().getUltrasonicRight() > 6
				&& Telemetry.getInstance().getUltrasonicRight() < 26; // CHANGE,
																		// ULTRASONIC
																		// VAL
	}
